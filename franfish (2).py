...
def age_history_table():
    if len(history_table) > 2000:
        for key in list(history_table.keys()):
            history_table[key] //= 2
...
def compute_think_time(wtime, btime, winc, binc, movetime, movestogo, turn):
    if movetime is not None:
        return max(0.02, (movetime / 1000.0) - 0.05)
    ...
    budget = remaining_sec / slices + increment_sec * 0.75
    return max(0.02, min(budget, remaining_sec * 0.4) - 0.05)
...
PST = {chess.PAWN: PAWN_TABLE, chess.KNIGHT: KNIGHT_TABLE, chess.BISHOP: BISHOP_TABLE, chess.ROOK: ROOK_TABLE, chess.QUEEN: QUEEN_TABLE}
    ...
    for sq in board.pieces(chess.PAWN, color):
        ...
        for f in (file - 1, file, file + 1):
            ...
            for enemy_sq in board.pieces(chess.PAWN, enemy):
                if chess.square_file(enemy_sq) != f: continue
                enemy_rank = chess.square_rank(enemy_sq)
                if color == chess.WHITE and enemy_rank > rank: blocked = True
                elif color == chess.BLACK and enemy_rank < rank: blocked = True
        if not blocked:
...
def development_score(board, color):
    ...
    for piece_type in (chess.KNIGHT, chess.BISHOP):
        for sq in board.pieces(piece_type, color):
            if chess.square_rank(sq) != back_rank: score += 10
    return score
...
def early_queen_penalty(board, color):
    ...
    if not queens or home_square in queens: return 0
    back_rank = 0 if color == chess.WHITE else 7
    developed_minors = sum(1 for pt in (chess.KNIGHT, chess.BISHOP) for sq in board.pieces(pt, color) if chess.square_rank(sq) != back_rank)
    if developed_minors < 2: return -30
    return 0
...
def hanging_penalty(board, color):
    ...
    for piece_type in PIECE_VALUES:
        if piece_type in (chess.KING, chess.PAWN): continue
        for sq in board.pieces(piece_type, color):
...
def evaluate(board):
    ...
    for sq in CENTER_SQUARES:
        if board.is_attacked_by(chess.WHITE, sq): score += 15
        if board.is_attacked_by(chess.BLACK, sq): score -= 15
...
def tt_lookup(key, depth, alpha, beta):
    ...
    if entry_depth >= depth:
        if entry_flag == EXACT: return entry_score, entry_move
        if entry_flag == LOWERBOUND and entry_score > alpha: alpha = entry_score
        elif entry_flag == UPPERBOUND and entry_score < beta: beta = entry_score
        if alpha >= beta: return entry_score, entry_move
    return None, entry_move

def order_moves(board, depth, tt_move=None):
    killers = killer_moves.get(depth, [])
    def score(move):
        if tt_move is not None and move == tt_move: return 1_000_000
        if board.is_capture(move):
            ...
            return 100_000 + v - a
        if move.promotion: return 90_000
        if move in killers: return 80_000
...
def negamax(board, depth, alpha, beta, deadline, allow_null=True):
    ...
    if board.is_repetition(3) or board.is_fifty_moves(): return 0
    ...
    for i, move in enumerate(moves):
        ...
        if i >= 4 and depth >= 3 and not is_capture and not move.promotion and not gives_check and not in_check:
            ...
            score = -negamax(board, depth - 1, -beta, -alpha, deadline) if reduced_score > alpha else reduced_score
        elif i == 0:
    ...
    flag = EXACT
    if best_score <= orig_alpha: flag = UPPERBOUND
        ...
        try:
            ...
            for move in order_moves(board, depth, best_move):
                ...
                if score > current_best_score:
                    ...
                    current_best = move
            if current_best: best_move = current_best
            depth += 1
            if depth > 80: break
        except TimeoutError:
            break
    ...
    while True:
        ...
        if cmd == "uci":
            ...
            if idx < len(tokens) and tokens[idx] == "moves":
                idx += 1
                for mv in tokens[idx:]: board.push_uci(mv)
        elif cmd == "go":
            wtime = btime = winc = binc = movetime = movestogo = None
            i = 1
            while i < len(tokens):
                if tokens[i] == "wtime": wtime = int(tokens[i + 1]); i += 2
                elif tokens[i] == "btime": btime = int(tokens[i + 1]); i += 2
                elif tokens[i] == "winc": winc = int(tokens[i + 1]); i += 2
                elif tokens[i] == "binc": binc = int(tokens[i + 1]); i += 2
                elif tokens[i] == "movetime": movetime = int(tokens[i + 1]); i += 2
                elif tokens[i] == "movestogo": movestogo = int(tokens[i + 1]); i += 2
                else: i += 1
            think_time = compute_think_time(wtime, btime, winc, binc, movetime, movestogo, board.turn)
            move = find_best_move(board, think_time)
            print(f"bestmove {move.uci()}" if move else "bestmove 0000")
            sys.stdout.flush()
        elif cmd == "setoption":
            pass 
        elif cmd == "quit":
